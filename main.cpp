...
int main(int argc, char** argv) {
    ...
    vector<int> iloscWarstNeuronow = {4,2,4};
    SiecNeuronow siec(iloscWarstNeuronow);
    Dane dane(4);
    srand(time(NULL));
     ...
     if(wybor ==1) {
         ...
         while (ktoraEpoka < ILOSCEPOK)
         {
             ...
             for (int i = 0; i < kolejnosc.size(); i++)
             {
                 ...
                 tmp = dane.pobierzWejscie(kolejnosc[i]);
             ...
             //tu daj zmiane wartosi wag by było offline
     ...
     } else
     {
        ...
        // dane.menuTestowanie();
         ...
         while(koniec)
         {
             ...
             double blad = siec.obliczBladDlaWzorca(tes);
            //cout << "\nBlad  wyliczenia sieci dla tego wzorca wynosi: " <<blad <<endl;
